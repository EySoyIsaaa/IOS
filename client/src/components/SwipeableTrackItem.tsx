...
export function SwipeableTrackItem({
  ...
  const leftIndicatorOpacity = Math.min(
    1,
    Math.max(0, -swipeX / SWIPE_THRESHOLD),
  );
  const rightIndicatorOpacity = Math.min(
    1,
    Math.max(0, swipeX / SWIPE_THRESHOLD),
  );
  ...
  return (
    <>
      ...
      <div ref={containerRef} className="relative overflow-hidden rounded-xl">
        ...
        >
          <div
            className={`${compact ? "w-10 h-10" : "w-12 h-12"} rounded-lg bg-zinc-800 overflow-hidden flex-shrink-0`}
          >
            <TrackArtwork
              src={track.coverUrl}
              alt={displayTitle}
